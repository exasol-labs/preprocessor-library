--/
CREATE OR REPLACE LUA SCRIPT PREPROC_RT.ERGONOMICS_TRAILING_COMMA_V2 AS
-- =============================================================================
-- ERGONOMICS_TRAILING_COMMA_V2 preprocessor module
-- (PREPROC_RT.ERGONOMICS_TRAILING_COMMA_V2)
-- =============================================================================
...
-- The boundary after a keyword is whitespace, ',', ';', a comment start, or
-- end-of-string. A '(' is NOT a boundary: a genuine list-terminating keyword is
-- never immediately followed by '(', whereas `set(...)`, `into(...)`, etc. are
-- ordinary function calls whose preceding comma must be kept.
--
-- CREATE ... SCRIPT is left untouched: its body after AS is a script language
-- (Python/Lua/R/Java), not SQL, so removing "trailing" commas there would
-- corrupt code (e.g. `zip(labels, set(labels))` -> `zip(labels set(labels))`).
-- The whole statement is returned verbatim.
--
...
function trailing_comma(sqltext)
    ...
    -- CREATE ... SCRIPT carries a script body (Python/Lua/R/Java) after AS that
    -- is NOT SQL; a comma there is code, never a trailing SQL-list comma. Leave
    -- the whole statement verbatim. The keyword SCRIPT appears in the leading
    -- clause (well within the first 256 chars), long before any body.
    local head = sqltext:sub(1, 256):upper()
    if head:match("^%s*CREATE%s") and head:find("%sSCRIPT%s") then
        return sqltext
    end
